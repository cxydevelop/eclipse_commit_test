 ...
 * @author 山人
 * 
 * @author xuying.chen 2017.3.16 判断空行优化
 */
public class XLSXCovertCSVReader {
		...
		public void startElement(String uri, String localName, String name, Attributes attributes) throws SAXException {
			...
			// c => cell
				...
				for (int c = 0; c < r.length(); ++c) {
					if (
						java.lang.Character.isDigit(r.charAt(c))) {
						firstDigit = c;
						break;
					}
				}
		...
		public void endElement(String uri, String localName, String name) throws SAXException {
			...
			if ("v".equals(name)) {
				...
				if (thisStr == null || "".equals(isCellNull)) {
					...
					record[thisColumn] = thisStr; // xuying.chen
													// null时不设置到record[]里面
				}
				...
				if (minColumns > 0) {
					...
					// xuying.chen 遍历record数组,拼接到一个字符串,判断字符串是否为空串
					String strIsNull = new String();
					for (String string : record) {
						if (string != null) {
							strIsNull += string;
						}
					}
					if (isCellNull == false && !"".equals(strIsNull.trim())) { // 判断是否空行
						strIsNull = null;
					...
					// if (isCellNull == false && record[0] != null && record[1]
					// != null)// 判断是否空行
					// {
					// rows.add(record.clone());
					// isCellNull = false;
	 ...
	 * @throws IOException
	 */
	public static List<String[]> readerExcel(String path, String sheetName, int minColumns) throws IOException, OpenXML4JException, ParserConfigurationException, SAXException {
	...
	// List<String[]> list =
	// XLSXCovertCSVReader.readerExcel("C:\\Users\\chenxuying\\Desktop\\员工信息批量导入或修改模板.xlsx","Sheet1",
	// 17);
